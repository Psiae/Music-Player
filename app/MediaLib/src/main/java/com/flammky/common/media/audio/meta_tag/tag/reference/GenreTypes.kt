 ...
 */
class GenreTypes private constructor() : AbstractIntStringValuePair() {
	//This maps the lowercase version to the id, so applications can map from the lowercase value to the id
	private val nameToIdMap: MutableMap<String?, Int?>
	...
	init {
		...
		nameToIdMap = LinkedHashMap(idToValue.size)
		for ((key, value) in idToValue) {
			nameToIdMap[value.lowercase(Locale.getDefault())] = key
		}
	}
	...
	fun getIdForName(name: String): Int? {
		return nameToIdMap[name.lowercase(Locale.getDefault())]
	}
	...
	companion object {
		 ...
		 * Winamp later.
		 */
		const val maxStandardGenreId = 125
		const val maxGenreId = 191
		private var genreTypes: GenreTypes? = null

		val instanceOf: GenreTypes
			get() {
				if (genreTypes == null) {
