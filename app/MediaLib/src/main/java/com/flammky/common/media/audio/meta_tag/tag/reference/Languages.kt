...
class Languages private constructor() : AbstractStringStringValuePair() {
	init {
		...
		idToValue.put(
			"\u0000\u0000\u0000",
			"Winamp Format"
		) //Not Part of Spec but commonly used by some applications
		idToValue.put(
			"XXX",
			"Media Monkey Format"
		) //Not Part of Spec but commonly used by some applications
	...
	companion object {
		const val DEFAULT_ID = "eng"
		const val MEDIA_MONKEY_ID = "XXX"
		const val WINAMP_ID = "\u0000\u0000\u0000"
		const val DEFAULT_VALUE = "English"

		//The number of bytes used to hold the language field size
		const val LANGUAGE_FIELD_SIZE = 3
		private var languageTypes: Languages? = null
		val instanceOf by lazy { Languages() }
	}
}
