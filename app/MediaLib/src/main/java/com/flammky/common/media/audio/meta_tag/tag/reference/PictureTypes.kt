...
class PictureTypes private constructor() : AbstractIntStringValuePair() {
	init {
	...
	companion object {
		private var pictureTypes: PictureTypes? = null
		val instanceOf: PictureTypes
			get() {
		...
		const val PICTURE_TYPE_FIELD_SIZE = 1
		const val DEFAULT_VALUE = "Cover (front)"
		const val DEFAULT_ID = 3
	}
}
