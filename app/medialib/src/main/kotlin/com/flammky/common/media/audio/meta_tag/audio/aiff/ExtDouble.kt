 ...
 */(var _rawData: ByteArray) {
	...
	fun toDouble(): Double {
		...
		exponent = _rawData[0].toInt() shl 8 or _rawData[1].toInt()
		exponent = exponent and 0X7FFF // strip off sign bit
		...
		var shifter = 55
		for (i in 2..8) {
			mantissa = mantissa or (_rawData[i].toLong() and 0XFFL shl shifter)
			shifter -= 8
		}
		mantissa = mantissa or (_rawData[9].toInt() ushr 1).toLong()

		// Now put it together in a floating point number.
		var `val` = Math.pow(2.0, exponent.toDouble())
		`val` *= mantissa.toDouble()
		if (sign != 0) {
			`val` = -`val`
		}
		return `val`
