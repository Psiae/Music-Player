		...
		FileChannel.open(file).use { fc ->
			...
			return if (frm8 != null) {
				DsdChunk.readChunk(Utils.readFileDataIntoBufferLE(fc, DsdChunk.DSD_HEADER_LENGTH))
					?: throw CannotReadException("$file Not a valid dff file. Missing 'DSD '  after 'FRM8' ")

				PropChunk.readChunk(Utils.readFileDataIntoBufferLE(fc, PropChunk.PROP_HEADER_LENGTH))
					?: throw CannotReadException("$file Not a valid dff file. Content does not have 'PROP'")

				SndChunk.readChunk(Utils.readFileDataIntoBufferLE(fc, SndChunk.SND_HEADER_LENGTH))
					?: throw CannotReadException("$file Not a valid dff file. Missing 'SND '  after 'PROP' ")
					...
					if (chunk is FsChunk) {
						...
						frte =
							try {
								...
								throw CannotReadException(file.toString() + "Not a valid dft file. Missing 'FRTE' chunk")
							}
				...
				if (chnl == null) {
					throw CannotReadException("$file Not a valid dff file. Missing 'CHNL' chunk")
				}
				if (fs == null) {
					throw CannotReadException("$file Not a valid dff file. Missing 'FS' chunk")
				}
				if (dst != null && frte == null) {
					throw CannotReadException("$file Not a valid dst file. Missing 'FRTE' chunk")
				}
				if (end == null && dst == null) {
					throw CannotReadException("$file Not a valid dff file. Missing 'DSD' end chunk")
				}
				val bitsPerSample = 1
				...
				throw CannotReadException("$file Not a valid dff file. Content does not start with 'FRM8'")
			} //end if frm8
		}
	}
