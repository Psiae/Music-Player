...
class LocalSourceImpl(
    ...
    override suspend fun fetchSong(): Flow<List<Song>> = flow {
        emit(queryDeviceSong())
    }

    suspend fun fetchSongFromDocument(uri: Uri) = flow {
        ...
        try {
           res.query(uri,
                ...
                for (i in 0 until cursor.columnCount) {
                    Timber.d("fetchSongFromDocument Column ${cursor.getColumnName(i)} = ${cursor.getString(i)}")
                }

               val dbg = "Uri = $uri, Authority = ${uri.authority} externalEnv = ${Environment.getExternalStorageDirectory()}"
                ...
                Timber.d("IntentHandler LocalSource $display_name $byte_size $identifier $dbg")
    ...
    private suspend fun queryDeviceSong() = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val songPath =
                if (VersionHelper.isQ())
                    MediaStore.Audio.AudioColumns.BUCKET_DISPLAY_NAME else MediaStore.Audio.AudioColumns.DATA
            val songPathId =
                if (VersionHelper.isQ())
                    MediaStore.Audio.AudioColumns.BUCKET_ID else MediaStore.Audio.AudioColumns.DATA
        ...
        deviceSong.forEach {
            Timber.d("DeviceSongQueried ${it.fileName} ${it.lastModified} ${it.byteSize} ${it.mediaId} ${it.mediaUri}, data = ${it.data}")
        }
        deviceSong
    }
