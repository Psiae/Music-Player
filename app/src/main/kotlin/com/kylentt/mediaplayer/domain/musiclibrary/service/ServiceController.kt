...
@Singleton
class ServiceController(context: Context, dispatchers: CoroutineDispatchers) {

	private val serviceDelegate: MusicLibraryService.ServiceDelegate?
		get() = TODO()

	private val controller = Controller(context, dispatchers)

	val playbackState
		get() = controller.playbackStateSF.asStateFlow()

	@MainThread
	fun sendControllerCommand(command: ControllerCommand) = controller.commandController(command)
	...
	private inner class Controller(
		...
		fun connectController(
			...
			futureMediaController = MediaController.Builder(context, sessionToken)
				...
				.buildAsync().apply {
					addListener(futureControllerListener, directExecutor)
					addListener( { onConnected(mediaController) }, directExecutor)
				}
		}
		...
		private fun setupMediaController() {
			Preconditions.checkState(::mediaController.isInitialized) {
		...
		private fun releaseMediaController() {
			...
			controller.release()
			Preconditions.checkState(!controller.isConnected)
