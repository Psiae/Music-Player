	...
	private object Delegate {
		...
		fun onEvent(activity: MainActivity, event: Event) {
			val state = when (event) {
				Event.Init -> State.Initialized
				Event.Create, Event.Stop -> State.Created
				Event.Start, Event.Pause -> State.Started
		...
		private fun updateStateDelegate(activity: MainActivity, state: State) {
			MainActivityDelegate.updateState(activity, state)
		}
	}
}
