...
class AuthData internal constructor(
	...
	private class AuthDataMapSerializer
		: KSerializer<Map<AuthSerializable, AuthSerializable>> {
		override fun deserialize(decoder: Decoder): Map<AuthSerializable, AuthSerializable> {
		...
		override fun serialize(
			encoder: Encoder,
			value: Map<AuthSerializable, AuthSerializable>
		) {
			return MapSerializer(
				keySerializer = AuthSerializable.serializer(),
				valueSerializer = AuthSerializable.serializer()
	...
	companion object {
		val UNSET = AuthData(Map(persistentMapOf()))
	}
}
...
abstract class AuthSerializable private constructor() {
	...
	companion object {
		val module = SerializersModule {
			InternalAuthModule.getAuthObjectSerializerModule()
